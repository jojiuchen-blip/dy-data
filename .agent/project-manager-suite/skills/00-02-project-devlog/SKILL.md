...
日志目录按以下优先级解析：命令行 `--devlog-dir`、`project-profile.md` 中的 `最近状态入口` / `状态入口`、默认 `logs/`。配置值必须是宿主根目录内的相对路径；运行日志与开发日志冲突时，应在画像中把开发日志映射到可跟踪目录，例如 `docs/devlog/`。
...
```bash
node <suite-path>/tools/devlog-sync.mjs <hostRoot> --title <标题> --goal <目标> --action <动作> --result <结果> [--devlog-dir docs/devlog] --json
```

通常不需要显式传 `--devlog-dir`：当项目画像已写入 `最近状态入口：docs/devlog/` 时，脚本会自动使用该目录；显式参数只用于一次性覆盖。
