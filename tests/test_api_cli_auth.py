...
def test_store_user_cli_auth_context_preserves_specified_store_scope(db_session) -> None:
    user = _add_user(db_session)

    auth = _auth_context_for_user(db_session, user)

    assert auth.role == "store"
    assert auth.store_scope_mode == "specified"
    assert auth.store_ids == ("store-1",)
    assert auth.has_global_data_access is False
...
@pytest.mark.parametrize(
    ...
    [("role", "admin"), ("password_hash", "changed-password-hash")],
)
