...
from apps.api.dy_api.models import (
    Base, ClueMaterializationWorkItem, DimStore, DimStorePoiMapping, JobImpact,
)
...
def poi_pg():
    ...
    try:
        Base.metadata.create_all(engine, tables=[
            DimStore.__table__, DimStorePoiMapping.__table__, JobImpact.__table__,
...
def test_concurrent_poi_insert_preserves_first_owner(poi_pg, contender: str) -> None:
    ...
    def compete() -> str:
        with poi_pg.begin() as session:
            contender_pid.append(session.scalar(text("SELECT pg_backend_pid()")))
            entered.set()
            try:
        ...
        with poi_pg.begin() as session:
            ...
            with poi_pg() as observer:
                while monotonic() < deadline:
                    ...
                    if blocked:
                        break
                    assert not future.done(), "contender finished without waiting on the winner"
                    sleep(0.02)
                else:
                    pytest.fail("contender did not enter a PostgreSQL lock wait")
