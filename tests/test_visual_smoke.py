...
@pytest.mark.parametrize("width,height", VIEWPORTS)
def test_dydata_81_store_dispute_entry_stays_collapsed_until_confirmation(
    browser: Browser,
    vite_live_admin_api_base_url: str,
    live_admin_fastapi_base_url: str,
    width: int,
    ...
    try:
        ...
        entry = page.get_by_role("button", name="发起账单异议", exact=True)
        expect(entry).to_be_visible()
        entry.click()
        confirmation = page.get_by_role("dialog")
        confirmation.get_by_role("heading", name="确认发起账单异议", exact=True).wait_for()
        ...
        output_dir.mkdir(parents=True, exist_ok=True)
        page.screenshot(path=output_dir / "store-settlement-dispute-entry.png", full_page=True)
    finally:
        context.close()
