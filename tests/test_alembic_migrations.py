...
def test_alembic_has_one_deployable_head() -> None:
    ...
    assert ScriptDirectory.from_config(config).get_heads() == ["20260909_0051"]
...
def test_existing_0036_database_can_upgrade_to_head(tmp_path: Path) -> None:
    ...
    with engine.connect() as connection:
        assert connection.execute(text("SELECT version_num FROM alembic_version")).scalar_one() == "20260909_0051"
