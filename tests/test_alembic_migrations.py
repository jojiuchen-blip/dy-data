...
def test_alembic_has_one_deployable_head() -> None:
    ...
    assert ScriptDirectory.from_config(config).get_heads() == ["20260916_0060"]
...
def test_clue_and_fee_branches_upgrade_to_shared_head(tmp_path: Path, starting_revision: str) -> None:
    ...
    try:
        ...
        with engine.connect() as connection:
            assert connection.execute(text("SELECT version_num FROM alembic_version")).scalars().all() == ["20260916_0060"]
    finally:
        engine.dispose()
...
def test_existing_0036_database_can_upgrade_to_head(tmp_path: Path) -> None:
    ...
    with engine.connect() as connection:
        assert connection.execute(text("SELECT version_num FROM alembic_version")).scalar_one() == "20260916_0060"
