...
def _auth(
    *,
    role: str = "store",
    store_ids: tuple[str, ...] = ("store-a",),
    store_scope_mode: str | None = None,
):
    resolved_scope_mode = store_scope_mode or (
        "all" if role == "highest_admin" else "specified"
    )
    return AuthContext(
        user_id="user-1",
        username="operator",
        ...
        store_ids=store_ids,
        auth_type="user",
        store_scope_mode=resolved_scope_mode,
    )
...
def test_global_store_list_resolves_all_current_stores() -> None:
    client, store = _client(_auth(role="highest_admin", store_ids=()))
...
def test_openapi_exposes_no_cli_business_writes_and_summary_has_no_detail_fields() -> None:
    client, _ = _client(_auth(role="highest_admin", store_ids=()))
