...
## 流程 9：DYDATA-19 月度账单、开票与财务查询（2026-08-20 loop 1）

```mermaid
flowchart LR
    A["门店 /settlement"] --> B["分别确认推广费与管理服务费"]
    B --> C["门店 /settlement/invoice"]
    C --> D["系统外开具推广费发票并登记"]
    D --> E["管理员 /finance/promotion 导入系统外审核结果"]
    B --> F["管理员 /finance/management 导入管理服务费发票及扣款"]
    E --> G["推广服务费订单明细"]
    F --> H["管理服务费订单明细"]
    E --> I["/finance/imports 只读审计"]
    F --> I
```

| 步骤 | 用户可见结果 | 运行证据 |
|---|---|---|
| 门店分别确认两个费用方向 | 两个方向独立确认，互不阻断 | Playwright 门店端 `/settlement` |
| 推广费登记 | 展示完整状态链；系统不创建开票申请或审核任务 | Playwright `/finance/promotion` |
| 管理服务费导入 | 只使用待开票/已开票，支持单店或导入覆盖的版本语义 | Playwright `/finance/management` |
| 指标查询 | 默认单月，可切换累计；已确认金额仅单月 | Playwright 点击“累计”并截图 |
| 导入与更正 | 展示首次、无变化、差异、整批失败和版本冲突 | Playwright 导入场景切换 |
| 穿透与审计 | 两类订单子路由独立，导入记录只读 | 自动化测试与稳定 URL |
