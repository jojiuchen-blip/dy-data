...
- Create: `alembic/versions/20260722_0019_add_cli_authorizations.py`
...
创建 revision `20260722_0019`，`down_revision = "20260721_0018"`。`upgrade()` 创建两个表及索引；`downgrade()` 先删 refresh 表再删 device 表；使用 `inspect(op.get_bind()).has_table(table_name)` 保护重复执行。
...
```powershell
git add apps/api/dy_api/models.py alembic/versions/20260722_0019_add_cli_authorizations.py tests/test_cli_auth_models.py tests/test_alembic_migrations.py
