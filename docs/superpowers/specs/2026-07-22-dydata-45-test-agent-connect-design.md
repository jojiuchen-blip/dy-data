...
- 动态注册只接受 public client 与精确 redirect URI；redirect URI 仅允许带 hostname 的 HTTPS，或 hostname 精确为 `127.0.0.1`、`localhost`、`::1` 的本机回环 HTTP，拒绝 fragment、userinfo、非法端口及其他 scheme；服务端不保存 OAuth client secret。
...
- 腾讯云测试入口先使用 DCR 16 KiB 应用层上限、元数据字段上限与 Nginx 按 IP 限流控制公开写入面；DYDATA-46 上生产前还必须补齐 OAuth 过期数据保留/清理任务、容量指标、告警阈值和生产流量下的限流参数，不得直接照搬测试值。
