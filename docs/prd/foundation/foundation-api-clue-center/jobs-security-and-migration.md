...
`/admin/sync` 同时承载订单、核销、退款、线索等多域采集能力。下列已存在接口继续由宿主同步模块维护，不进入线索中心 23 张目标表的 C2/C3 Schema 分母，但属于冻结交互的 C6 支撑接口：

| 交互 | 共享接口 | 边界 |
|------|----------|------|
| 刷新同步状态与日志 | `GET /api/v1/admin/sync` | 读取宿主任务、调度配置和日志 |
| 保存通用同步配置 | `PUT /api/v1/admin/sync/config` | 保存多域采集配置，不写线索业务事实表 |
| 提交手动补拉 | `POST /api/v1/admin/sync/run` | 创建宿主同步任务，完成后再由 J01/J02 消费证据 |

现有 `POST /api/v1/admin/sync/clue-center/rebuild` 在目标契约中删除，由 J07 预览和 J08 确认执行替代。线索中心维护按钮可以在一次受控交互中先调用 J07、确认影响后调用 J08，但不得绕过预览直接重建正式数据。
