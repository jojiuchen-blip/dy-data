...
| # | 需求 | 需求简述 | 对应章节 |
|---|------|---------|---------|
| R1 | 直接访问与来源上下文恢复 | 直接访问时展示当前账号授权范围；从单店分账进入时恢复账单/预览、门店、月份、产品、费用方向、费率和规则版本上下文，并由服务端重新授权和校验。 | §3 |
...
│ 主模块侧栏 / 门店结算顶栏 / 四页导航                            │
├──────────────────────────────────────────────────────────────────┤
│ 订单费用明细标题 · 当前授权范围/已验证来源 · 条件返回单店分账    │  ← 本文档 §3
├──────────────────────────────────────────────────────────────────┤
...
```
[订单费用明细]
[当前账号授权范围 / 来源：单店分账] [门店] [账期/预览月份] [产品范围 · 商品类型]
[费用方向] [费率集合] [规则版本集合]             [有来源时返回单店分账]
```

**交互规则**：

- 页面加载时始终请求明细；无来源参数时按当前账号授权门店范围查询当前结果并显示正常列表或空态，不显示“返回单店分账”。
- 存在完整来源上下文时恢复下钻语义：`statementId` 存在时按账单冻结来源查询，`statementLineId` 必须属于该账单；具备合法 `storeId + month` 时按当前结果指针查询只读预览，并显示返回入口。
- URL 中的月份、门店、产品、费用方向、费率和规则版本只用于恢复筛选和校验，不授予权限，也不作为费用重算输入。
- 服务端返回的 `context` 是页面唯一可信回显；无来源访问使用接口默认值并明确显示“当前账号授权范围”。非法或过期的完整账单来源不得静默回退到默认查询。
...
2. 有 `statementId` 时校验 `statementLineId` 归属，并从 `settlement_statement_entry` 读取已冻结的结果或调整来源；无账单时从 `settlement_fee_result_current` 解析当前不可变结果版本，若未提供 `storeId/month` 则仅按登录账号授权门店范围查询。
...
| `storeId` / `month` | PRD 2 单店分账当前筛选 | 可选；用于缩小只读预览范围，缺省时使用账号授权范围 |
...
| 无账单且未提供 `storeId` / `month` | 按当前账号授权门店范围返回默认列表；任一参数存在时仅作为额外缩小范围的筛选。 |
...
- HTTP 响应头写入筛选摘要和生成时间；文件名使用安全业务日期，不包含门店隐私信息、本地路径或凭据。
...
按当前账号授权范围或已授权来源上下文、费用方向和筛选条件返回一券一方向费用明细。

> 完整接口契约见[双费用结算与报表 API §3](../foundation/foundation-api-dy-data/settlement-reporting.md#3-get-apiv1order-fee-details--订单费用明细)。

**请求参数来源**：

- `statementId/statementLineId` 或 `storeId/month`：可选，来自 PRD 2 的账单行或预览汇总行；两种口径不得混用为权限依据；直接访问时均可缺省。
