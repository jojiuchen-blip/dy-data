...
1. 校验门店、账期、当前账单版本及推广费有效确认。
2. 校验 20 位数电专票号码、日期、金额和读取版本。
...
| 2 | 业务规则 | 一账期一票 | 同门店同账期重复登记 | 仅一张当前有效发票，历史版本可查 |
...
完整契约见 [账单发票 API §4](../foundation/foundation-api-dy-data/billing-invoice.md#4-发票登记与财务查询)。请求不含附件、备注、原发票 ID 或跨账期分配。
