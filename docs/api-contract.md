...
- 第一阶段可以不依赖退款接口。订单接口已返回订单状态和券状态，足以支持页面判断 `none`、`refunding`、`refunded` 并排除退款券/订单。只有页面需要售后单号、退款原因、审核状态、退款完成时间或退款金额拆分时，才需要增加退款接口采集。
- 若后续接入退款接口，退款接口缺少稳定券 ID；页面可展示退款状态和金额，但一单多券场景下券粒度归属需标记为推断。
- 门店筛选使用内部 `store_id`，后端需要先完成 POI 到内部店的映射表。
- 销售归属门店建议后端先按订单归属人 UID 匹配，未命中时用订单归属人昵称补充；UID 和昵称均未命中的订单应单独输出未匹配清单，避免静默归错门店。
