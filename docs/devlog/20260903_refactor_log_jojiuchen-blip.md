...
## 2026-09-04 长任务失联与持久恢复修复

- 生产现象：任务 `admin_sku_fee_rules-c983afef58d22133eded232d` 从 13:58 持续显示 `running / 0`，超过既有 332,019 券全量重算的最长成功耗时；活动投影未发布，因此 `/ranking?period=2026-08` 和单店月度分账仍读取旧空代际并显示 0。
- 最终根因：分佣发布接口把 30 万级全量结算注册为 FastAPI `BackgroundTasks`，实际在 API 容器内执行。API 容器仅 768MB，且临时任务不具备进程重启恢复能力；执行器退出后，旧任务没有 claim token、lease 或 heartbeat，Worker 看到任意 `running` 又直接返回，形成永久阻塞。
- API 修复：新旧分佣规则保存、例外账号保存和批量导入提交现在只原子写入规则与 `queued` 任务，不再从 HTTP 请求生命周期启动结算重算。
- Worker 修复：原子认领时写入 fencing token、15 分钟租约、attempt 和运行阶段；两个 30 万级逐券阶段以及投影阶段按有界进度续租。失联租约由 Worker 以 CAS 恢复为 `queued`，最多尝试 3 次；有新鲜租约的执行者不会被误抢。
- 终态修复：旧聚合事务提交后任务继续保持 `running`；只有 sparse overlay 校验、活动指针 CAS 发布及元数据写入全部完成，才清理租约并写 `success`。发布失败或租约丢失不会留下假成功。
- 发布安全：腾讯部署顺序改为先强制替换并健康检查 API，再启动支持服务和新 Worker，确保本次遗留 API 执行器已退出后才恢复旧任务，避免旧、新执行器并行写结算表。
- TDD 证据：修复前 6 个回归点全部失败；修复后 API 入队、运行态、发布终态、失联恢复、活跃租约保护、重试耗尽和部署顺序用例均通过。专项结果：管理员分佣接口 `36 passed`、旧管理员规则接口 `13 passed`、结算计算与增量结算 `90 passed`、部署合同 `23 passed`、结算重建队列 `8 passed`。
- Foundation 漂移：扩充既有 `S4-FCR-011`，记录 Worker-only、租约心跳、失联恢复和发布后成功合同；不直接改写 Foundation 正文。
- 当前状态：本地实现与专项验证完成，尚待 GitHub 全量门禁、生产发布、遗留任务自动恢复及榜单/单店分账在线验收。

### 第二轮并发与恢复审查补强

- 单条 SKU 费率规则与结算范围规则也改为在规则提交的同一数据库事务内创建确定性重算任务；同幂等键重放会补齐历史缺失任务，不再依赖前端随后发起的第二个请求保证重算。
- Worker 仅在认领短事务中持有 PostgreSQL transaction advisory lock；SQLite 测试环境使用进程锁。不同任务的选择和认领串行化；长任务以持久租约和提交围栏保护，不持有会阻塞失联恢复的长会话锁。
- claim token 已贯穿 sparse generation 创建、月度分区、榜单分区、finalize、ready 和活动指针发布。每次独立提交前都以数据库时钟锁定并验证任务租约；过期执行者不能续租、提交分区或写成功终态。
- Worker 无论自动同步是否开启都会按短轮询消费持久化重算队列；普通异常在 3 次预算内进入 `retry_wait`，退避到期后回到 `queued`，进程失联由租约扫描恢复。发布事实通过持久审计事件对账，不依赖当前活动指针仍指向该任务；旧版本留下的“成功但未发布”管理员任务会只恢复一次。
- 新增故障注入覆盖：单条规则/范围规则原子入队、不同任务并发互斥、过期 claim 拒绝续租与完成、sparse 提交围栏、普通异常重试、发布后崩溃对账、旧提前成功恢复、自动同步开启时持续消费。
- 第二轮专项验证：上述新增故障注入 `8 passed`，租约与活动代际关键路径 `11 passed`，Worker 调度全文件 `40 passed`，规则/榜单代际组合 `143 passed`，结算与增量投影 `98 passed`；九个直接受影响文件组合回归 `304 passed`。完整 pytest 与 Web production build 仍在发布前门禁中。
