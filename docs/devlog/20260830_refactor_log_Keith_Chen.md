...
- **2026-09-02 / DYDATA-86：分佣规则发布记录存在但已启用商品列表为空**
  - **根因**：规则发布成功后页面只刷新费率规则，SKU 商品列表没有按当前生效规则补查初始列表之外的 SKU；确认弹窗失败信息还被写到弹窗后的页面通知中。
  - **修复**：规则/商品数据统一刷新并用精确 SKU 查询合并缺失行；发布记录补充 SKU ID；确认弹窗显示逐条进度和可重试错误。
  - **验证**：规则页/API 回归 15 项通过，视觉 smoke 2 项通过，Web production build 通过；本任务无 foundation 漂移。
