...
| PAGE_EXPLAINER | present | src/frontend/page-preview/ | 无 | 已发现页面交互说明文件 |
| FOUNDATION | missing | docs/prd/foundation/ | foundation-builder | 缺少术语表、Schema、API 与交付清单；代码中发现 API 端点线索 64 个、模型定义 37 个、迁移 19 个 |
...
- 配置线索：apps/web/tsconfig.json、apps/web/tsconfig.tsbuildinfo、apps/web/vite.config.ts、config.example.json、config.local.json、deploy/.env.example、src/dy_data/config.py
