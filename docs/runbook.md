...
绑定采集检测到抖音登录页时，任务失败原因记录为 `douyin_backend_login_required`。后台“数据同步”的任务列表和最近失败提示显示“抖音后台登录已失效，请在受保护的浏览器中重新登录后重试”。管理员应进入受保护浏览器，完成抖音后台登录，再重试绑定同步并确认任务成功和数据更新时间；无需清空浏览器资料或重新配置开放平台密钥。该提示只用于明确检测到的登录失效，不把普通超时、接口地址未发现或 HTTP 错误统一归为登录问题。历史模糊错误不自动改写。
...
| 状态 | 初始条件 | 行为 |
|---|---|---|
| 正常 | 已通过恢复观察，未发生持续压力 | 日批、必要采集正常；空闲时按日补历史 |
| 收紧 | 可用内存 < 2 GiB 或 worker 容器使用率 > 80%，连续 60 秒 | 不启动新历史任务，运行中历史在安全检查点让出；日批可继续 |
| 保护 | 可用内存 < 1 GiB 或 worker 容器使用率 > 90%，连续 30 秒 | 不启动新重任务；已有任务在页、阶段或发布事务边界让出 |
| 恢复观察 | 可用内存 > 2.5 GiB 且 worker 容器使用率 < 75%，连续 180 秒 | 先放行日批和必要采集，再连续健康 600 秒放行历史 |

静态 swap 占用不再单独触发暂停，也不要求 swap 清零。监控 `/proc/vmstat` 的实际换入换出速率；初始以 > 32 MiB/s 且可用内存 < 2 GiB、连续 30 秒升级保护。该阈值需生产负载观察校准。主机可用内存反映共享主机压力，容器比例专指 worker，不能解释为已经独立监控所有服务容器。

进程启动先进入保护并重新建立 180 秒健康观察，避免重启绕过滞回。原有进程 RSS 和容器绝对硬上限保留；硬 RSS 强制终止仍按真实失败处理。新分级阈值使用 `WORKER_RESOURCE_PRESSURE_*` 变量；旧 host-used / max-swap 设置不控制新状态机。配置错误和缺失关键采样不得当作资源安全。

软暂停错误码为 `worker_resource_pause`，60 秒后重新具备调度资格，但必须再次通过资源准入。已提交页和阶段不回滚、不重复计数。为保持现有数据库结构，`quota_pause_count` 现在统计预算和资源两种调度让出；对应 attempt 必须带各自的错误码并通过历史一致性审计，两类让出均不消耗三次真实失败预算。

后台展示资源状态、原因、持续时长、恢复条件及同步时效。资源心跳和具体任务心跳分开读取；独立监控的空闲字段不能掩盖执行中的重任务。恢复验收必须同时检查：资源状态已放行、原排队任务真正开始、日批采集及最终发布完成、绑定数据重新更新。不得通过手动清空任务状态、删除检查点或日常执行 swapoff 来代替策略修复。
