...
(function (global) {
  ...
  const INK   = C["1C1C1A"];   // 墨：主数据、标题、强调
  const PAPER = C["F0EFEB"];   // 纸：页面底色 = 浅卡底色（卡片无边框，靠留白分卡）
  const MUTED = C["8F8E88"];   // 次级文字、副标题
  const FAINT = C["C6C5BF"];   // 来源行、辅助刻度
  const GRID  = C["DEDDD6"];   // 网格线、发丝线

  // 7 级灰阶 ladder：多系列时按重要性从黑到浅分配
  const L=[...window.DC_LIEFLAT_THEME.ladder];
  // 5 级简版（waffle 等少系列场景）
  const LAD=[...window.DC_LIEFLAT_THEME.ladderCompact];

  // 暗卡（dark card）专用：底 #1C1C1A，其上的"墨"反转为纸色
  const DARK = {
    bg: C["1C1C1A"],
    ink: C["F0EFEB"],          // 暗卡上的主数据色
    muted: C["8F8E88"],
    faint: C["55554F"],        // 暗卡来源行
    grid: C["2E2D29"],         // 暗卡网格 / 发丝线
    gridSoft: C["2A2925"],
    ladder: [ACCENT_ON_DARK, C["DCDAD2"], C["C9C7BD"], C["B3B0A4"], C["8F8E88"], C["6A6963"], C["4A4944"]],
  };
  ...
  *{margin:0;padding:0;box-sizing:border-box}
  body{background:var(--bg);font-family:var(--font-sans);color:var(--ink);padding:40px;-webkit-font-smoothing:antialiased}
  ...
  .ch{height:320px}
  svg text{font-family:var(--font-sans)}` + MOTION.css;

  global.MONO = { ACCENT, ACCENT_ON_DARK, INK, PAPER, MUTED, FAINT, GRID, L, LAD, DARK,
