...
GitHub 腾讯云成功发布记录 `34100212205` 的版本为 `f9b044f`，包含已上线 DYDATA-87 修复，但远端 main 仍为 `86a8171`。本次发布先合入该生产基线，避免回退线上结算行为。业务源码自动合并无冲突；执行入口保留两项任务，引用索引由生成器重建。此前 2418 项本地通过记录对应原始功能基线，合并后的版本必须通过新的完整 CI 和生产发布门禁；发布及回滚应以实际生产 SHA 为准。

合并后本地验证：引导、线索与设计系统相关测试 136 passed（123.67 秒），Web production build 通过；API、Worker、领域代码和部署文件相对 `f9b044f` 无差异，引导组件、编排和页面接入相对 `e821f90` 无差异。计划结构 13/13 与阶段门禁通过；完整 CI 和发布证据继续回填 Linear。
