...
| T5.7 | [系统测试、v2-clean 一致性与生产验收](sub-delivery-plan-dy-data-T5.7-system-uat.md) | AI 执行 -> 人审核 | T5.1～T5.6 | 进行中（DYDATA-80/81 主应用一致性与生产交付） | - | G1/G2/G3 已关闭且最终复审 Ready；当前冻结 v2-clean、逐页映射 `apps/web`、执行产品一致性与全量发布门禁。Owner 已授权全部硬门禁通过后无需二次确认直接部署，任一门禁失败即停止并记录证据 |
