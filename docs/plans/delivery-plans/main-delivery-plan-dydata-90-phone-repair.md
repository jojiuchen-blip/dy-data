...
| 差距 | 影响 | 对应任务 | 状态 |
|---|---|---|---|
| 已分配线索缺可用号码缓存 | 列表和号码读取为空 | T0.5 | 生产验证完成，待用户验收 |
...
| Task | 子开发计划 | 状态 | 完成日期 |
|---|---|---|---|
| T0.5 | [sub-delivery-plan-dydata-90-phone-repair-T0.5.md](sub-delivery-plan-dydata-90-phone-repair-T0.5.md) | 生产验证完成，待用户验收 | 2026-09-10 |
...
- [x] 专项和真实PG回归通过：号码恢复、真实PG与API共50项通过，API无抖音凭证时可消费worker恢复后的缓存。
- [x] 全仓、构建、CI与备份门禁通过：CI 2773 passed/161 skipped；本地全仓原1项SQLite夹具失败修正后整组浏览器256 passed，专项50 passed；Web构建通过。
- [x] 生产号码可用、无重复分配、自动超期关闭：2204条有效可见线索缓存缺口219→0，完整号码页面读取通过。

发布：`1ec89c0cffb1c6e48e30d3826bec30a2cfb3d874`，流水线`34458749861`于上海2026-09-10 17:52成功。18:48回读时后台已恢复218条，使用已部署的同一有界恢复入口补齐余下1条；7个worker源码哈希与提交一致。备份为`pre-production-cutover-20260910T094936Z.env`和`pre-migrate-20260910T094941Z.dump`（682183232字节、0600），回滚镜像标签`rollback-phone-20260910`。
