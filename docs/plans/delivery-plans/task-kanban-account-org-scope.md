# 账号组织权限与门店选择 Task Kanban

| Task | 子开发计划 | Owner | 前置 | 状态 | 完成日期 | 备注 |
|---|---|---|---|---|---|---|
| T0.1 | [sub-delivery-plan-account-org-scope-T0.1-account-scope.md](sub-delivery-plan-account-org-scope-T0.1-account-scope.md) | AI | 无 | 已完成 | 2026-09-15 | PR #33、生产eeca61e、部署34928814248成功 |
| T0.2 | [sub-delivery-plan-account-org-scope-T0.2-default-password.md](sub-delivery-plan-account-org-scope-T0.2-default-password.md) | AI | T0.1 | 进行中 | - | 用户追加：批量开户初始密码统一123456，后续自行修改 |
