...
**完成证据**：增量影响闭包、keyset 分页、批次提交、观察版本防回退、标识冲突 fail-closed、SQL 聚合和线索中心有界投影已集成；增量专项 `45 passed`，线索中心/分配回归 `64 passed`，控制面相关专项 `182 passed`，Alembic 全链 `54 passed`。真实 PostgreSQL 锁竞争及 4C/8GB RSS 门禁保留到 T2.4 执行。
