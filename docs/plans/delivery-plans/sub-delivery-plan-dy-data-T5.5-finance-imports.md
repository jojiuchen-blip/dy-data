...
**核心逻辑**：
- 实现接口 #37～#42 和基础信息、推广服务费厂家结果、管理服务费厂家结果、SAP 确认四类模板；管理费发票与厂家扣款为同一模板事实。
...
**完成证据**：最终四类模板与 6 个接口覆盖五种结果；`tests/test_api_finance_imports.py` 5 passed，`tests/test_api_store_billing.py` 22 passed，Alembic 单一 head 与 0033～0036 往返 2 passed；5,001 行请求返回 413 且批次/行记录均为 0；上传/提交幂等、全错误下载、原子写入和版本冲突均有回归；`S4-FCR-003`、`S4-FCR-004` 已修订并关闭。受控内存证据 160.37 MiB 为同一 5,000 行上限解析器的既有记录。
