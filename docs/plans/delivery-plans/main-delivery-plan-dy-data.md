...
> **当前需求基线**：Linear DYDATA-19、DYDATA-31、DYDATA-80、DYDATA-81 及 2026-08-30 用户书面裁决；2026-08-20 已确认的 mainprd、9 份 subprd、Foundation，冻结原型 `codex/dydata-19-finance-mock@9a574fa`、`docs/prototypes/dydata-19-finance-flow-dashboard/`，以及 2026-08-28 已确认的财务一级导航设计
> **上游发现结论**：`collect-upstream-context.mjs` 于 2026-08-30 返回 `canProceed=true, slug=dy-data, mode=pipeline`；无必需产物缺失；G5 读取 SubPRD 2、5～9、Foundation、冻结原型、DYDATA-81 合同矩阵和正式 API/Schema 证据

## 0. 本计划使用指南

1. 先读取本主计划和任务看板，只选择依赖已满足且状态为 `待开发` 的 Task。
2. 再读取该 Task 的子计划、其中列出的 PRD 章节、Linear issue、真实代码和测试文件。
3. T1.1～T3.3 是已完成历史能力，T4.1 保留原发布依赖；本次新增 T5.1～T5.7，初始均为 `待审阅`，人类 Owner 审阅通过前不执行代码。
...
- `docs/plans/2026-08-30-dydata-81-finance-contract-controller-spec.md` 冻结 G5 的控制器边界、已确认口径、假设、测试包、发布门禁和待裁决项；不得以其替代 Linear 或 Foundation 的业务权威。
...
| 账单确认、异议、发票版本、状态事件、财务导入与审计表尚不存在 | P0 | DYDATA-19 无可靠写入事实与版本追溯 | T5.1 | 已完成（2026-08-21） |
...
| 六个财务页面的内部模块顺序、筛选、表头、状态、导入动作、空错态与跳转仍未逐行满足冻结合同 | P0 | 构建可用但不能通过业务 UAT，可能遗漏 SAP、结算状态与异常证据 | T5.7 G5 | 本地实现与三视口隔离 UAT 已完成；生产门禁阻塞 |
| 财务导入值尚未形成“门店原值 / 财务值 / 当前有效值 / 操作人 / 时间 / 版本”的完整查询与单条矫正闭环 | P0 数据正确性 | SAP 生效来源不唯一或历史不可追溯 | T5.7 G5 | 本地闭环已完成并通过版本/审计回归；目标 PostgreSQL 门禁待执行 |
| 账单异议“系统检测中 / 查看检测进度”缺少正式异步任务、持久化进度、结果和失败原因 | P0 | 刷新后状态丢失或以假动作误导用户 | T5.7 G5 | 正式异步任务本地闭环已完成；自动检测不裁决异议，生产依赖 worker 与目标环境门禁 |
| DYDATA-82 证明材料受控上传尚未完成定义 | 不再属于本轮发布依赖 | 2026-08-31 用户裁决取消异议文件上传，reason-only 请求由正式 API 保存具体原因；历史字段仅兼容读取 | T5.7 G5 | 已由用户裁决解除对象存储门禁；reason-only API/UI/UAT 证据已通过 |
...
边界：本计划不执行真实开票或资金划拨，不创建开票申请单或系统内厂端审核任务，不调用外部验真或企业微信，不扩建新的财务角色；DYDATA-19 仅开放已冻结的确认、异议、发票信息登记、查询、导入和审计接口。
...
| Task | 子开发计划 | 状态 |
|---|---|---|
| T5.1 | [财务闭环 Schema 与领域地基](sub-delivery-plan-dy-data-T5.1-finance-schema.md) | 已完成（2026-08-21） |
| T5.2 | [门店确认与推广费发票登记](sub-delivery-plan-dy-data-T5.2-store-billing.md) | 已完成（2026-08-21） |
| T5.3 | [管理员财务查询与订单穿透](sub-delivery-plan-dy-data-T5.3-finance-queries.md) | 已完成（2026-08-21） |
...
| T5.7 | [系统测试、v2-clean 一致性与生产验收](sub-delivery-plan-dy-data-T5.7-system-uat.md) | 进行中（G4 已部署至 `df617e7`；G5 本地实现/UAT 已完成，生产放行被硬门禁阻塞） |
...
- [ ] 【用户确认，2026-09-09，DYDATA-87】两费均须有效核销，按有效核销月计入、有效核销日匹配各自费率版本、同一核销实收基数计算；推广归销售门店，管理归核销门店。仅有效商品、合规订单归属账号和直播/短视频渠道纳入；不以商品归属替代订单归属。
- [ ] 部分/全额退款和取消核销形成独立调整；原结果、已锁账账单和规则版本不被覆盖。
- [ ] 全国榜单、单店分账、订单费用明细和导出均服务端重验权限；全国前 20 例外不扩展明细权限。
- [x] 开票页仅登记系统外已开具的推广费发票事实，不创建申请单、不执行开票或审核；旧五节点静态页已删除。
...
- [ ] 推广服务费、管理服务费、订单明细、门店基础信息、SAP/账单异议、导入记录逐行满足冻结合同；未确认字段或正式 API 缺口不得以演示值填充。
- [ ] 推广费 5 张指标卡口径一致，其中“待开票金额 = 审核未通过金额 + 账期未开票金额”；页面、列表、导出使用同一服务端口径。
- [ ] 财务导入和单条 SAP 矫正均以财务值更新当前有效 SAP，并保留门店原值、财务值、操作人、时间、版本和操作审计；历史账单与订单快照不回写。
- [ ] 账单异议异步检测的状态、进度、结果与失败原因来自正式持久化 API，刷新可追溯；自动检测不得自动决定异议成立、驳回或结算结果。
- [ ] 六页在 1440、768、390 三档完成正式 API UAT、Given/When/Then、空态/错态/无权态、导入/导出/跳转和无全局重叠验证。
- [x] DYDATA-82 对本轮正式异议闭环的发布影响已有书面裁决：2026-08-31 取消文件上传，改为填写具体原因；新建异议不依赖对象存储。
...
| 财务合同实现引入正式字段或状态而 Foundation 未同步 | API、页面与审计口径漂移 | 先以失败契约测试锁定，再同步 Foundation/API/Schema；无正式来源的字段保持 BLOCKED | AI 执行 -> 人审核 | G5 进行中 |
| DYDATA-82 证明材料需求未定义 | 旧文档曾要求异议附件 | 用户已裁决取消文件上传；新建异议改用具体原因，reason-only 回归与 UAT 已完成 | 人类 Owner + AI | 已解除对象存储阻塞 |
...
| DYDATA-19；Foundation Schema 账单发票 §1-§8 | DYDATA-19-SCHEMA | T5.1 | [T5.1](sub-delivery-plan-dy-data-T5.1-finance-schema.md) |
| SubPRD 2、4；Foundation API #23-#30 | DYDATA-19-STORE-BILLING | T5.2 | [T5.2](sub-delivery-plan-dy-data-T5.2-store-billing.md) |
| SubPRD 5～7；Foundation API #31-#34 | DYDATA-19-FINANCE-QUERY | T5.3 | [T5.3](sub-delivery-plan-dy-data-T5.3-finance-queries.md) |
