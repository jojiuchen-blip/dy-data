...
| 账单异议“系统检测中 / 查看检测进度”缺少正式异步任务、持久化进度、结果和失败原因 | P0 | 刷新后状态丢失或以假动作误导用户 | T5.7 G5 | 正式异步任务本地闭环已完成；自动检测不裁决异议，生产依赖 DYDATA-82 与 worker 门禁 |
| DYDATA-82 证明材料受控上传尚未完成定义 | P0 发布依赖 | 若正式异议闭环要求上传，则缺少存储、安全、留存和审计合同 | T5.7 G5 | BLOCKED；实现可继续，生产放行前必须明确本轮是否包含并关闭相应门禁 |
...
- [ ] DYDATA-82 对本轮正式异议闭环的发布影响已有书面裁决；若属于必需范围，则其受控上传、安全、留存、审计和回归必须在生产部署前通过。
...
| DYDATA-82 证明材料需求未定义 | 异议提交可能缺少正式证据链 | 代码实现与 UAT 可继续，但生产放行前由 Owner 明确本轮是否需要；需要则先完成安全设计与实现 | 人类 Owner + AI | 阻塞生产裁决 |
