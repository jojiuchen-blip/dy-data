...
| 系统仍无门店确认/异议/推广费发票登记与管理员财务接口 | P0 | 页面只能展示旧只读数据 | T5.2～T5.5 | T5.2 已完成；T5.3 进行中；其余待审阅 |
| `/invoice` 仍保留旧五节点流程，管理员财务页面尚未进入生产应用 | P0 | 与已冻结的系统外开票边界冲突 | T5.6 | 待审阅 |
| 新财务闭环尚未完成真实浏览器、并发、原子导入与用户验收 | P0 | 不满足发布条件 | T5.7 | 待审阅 |
...
| T5.4 | [账单异议生命周期](sub-delivery-plan-dy-data-T5.4-disputes.md) | 进行中 |
| T5.5 | [四类财务导入与更正](sub-delivery-plan-dy-data-T5.5-finance-imports.md) | 待审阅 |
| T5.6 | [生产页面与跨页流程](sub-delivery-plan-dy-data-T5.6-finance-pages.md) | 待审阅 |
| T5.7 | [系统测试与用户验收](sub-delivery-plan-dy-data-T5.7-system-uat.md) | 待审阅 |
...
| 旧 `/invoice` 五节点语义残留 | 用户误以为系统内执行开票和审核 | T5.6 删除旧语义并以契约测试、浏览器截图锁定系统边界 | AI 执行 -> 人审核 | 待处理 |
| 大文件导入错误量导致内存峰值 | 服务不稳定或错误被截断 | T5.5 流式解析、错误落库、分页查询与下载；压测峰值并禁止一次性装载 | AI 执行 -> 人审核 | 待处理 |
| 账单/发票/导入并发覆盖 | 财务历史不可追溯 | 所有写入使用读取版本、幂等键和原子事务；409 返回当前版本及最近操作 | AI 执行 -> 人审核 | 待处理 |
