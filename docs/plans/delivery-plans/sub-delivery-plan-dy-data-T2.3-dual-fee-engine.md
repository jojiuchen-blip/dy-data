...
**核心逻辑**：
- 【用户确认，2026-09-09，DYDATA-87 增量覆盖】两费均须有效核销，按有效核销日匹配各自费率、有效核销月计入、同一核销实收基数计算；推广归销售门店，管理归核销门店。只纳入有效商品、合规订单归属账号、直播/短视频渠道，不按商品归属替代订单归属。
- 按规则匹配日选取不晚于该日的最新 ACTIVE 双费率版本；冲突或缺失进入数据质量阻断。
- 一券一方向生成不可变结果；未锁账重算新增版本后事务切换当前指针，不覆盖旧结果。
- 部分退款同比减少两方向共同基数、全额退款归零、取消核销影响两方向；后续事件新增调整并进入事件发生月份，历史取消不得抵消后续重核销结果。
...
**完成标准**：
- 增量验收：8 月销售、9 月有效核销时两费均入 9 月并使用核销日版本；销售与核销实收不同仍使用同一核销实收基数，同券同费率两费相等；不改写已发布费率、历史结果与冻结账单。
...
> 上述状态保留 T2.3 原交付历史，不代表 2026-09-09 新口径已部署或验收；本次增量由 DYDATA-87 / T5.7 承接，发布验证和 FCR013 状态由主控收口。
