...
- 当前 Task：T0.1，进行中。
- 用户已授权实施、跳过Linear、同步主分支与服务器部署。
- 进度：四项业务变更已实现，独立审查两项重要问题已修复；专项测试、构建、治理与本地真实后端页面/Excel验收通过。全量测试与发布待完成。
- 证据：[开发日志](../devlog/20260924_clue_product_scope.md)。

# 当前执行：DYDATA-97 费用明细导出恢复与佣金按接口订单实收计算（2026-09-23）

- 当前阶段：S4。
- 当前正式计划文件组：[主计划](delivery-plans/main-delivery-plan-dydata-97-fee-fixes.md)、[任务看板](delivery-plans/task-kanban-dydata-97-fee-fixes.md)。
- 当前子开发计划：[T1.1 导出恢复与实收基数](delivery-plans/sub-delivery-plan-dydata-97-fee-fixes-T1.1-receipt-amount.md)。
- 当前 Task：T1.1，进行中；owner 为 dsh执行 / Codex技术验收 / 用户业务验收。
- 范围：订单分佣页与财务各页全部导出/下载模板/下载错误行按钮逐项检查（主范围 14 个 + 相关页面 11 个，共 25 个）；三类导出端点（`/admin/finance/invoices/export`、`/admin/finance/order-details/export`、`/order-fee-details/export`）先复现再最小修复；佣金与费用基数优先取接口订单/券明确 `receipt_amount`，保留 0 与缺失语义，实付字段原含义保留，legacy 佣金链分离基数；确定缺陷最小修复后按用户授权部署（由 Codex 执行验收与部署）。
- 边界：无数据库迁移；用户已授权修复后部署（Codex 执行）；不自动重算历史锁账与生产账单，已锁账快照只读不修改；不以增加列或按钮代替导出故障复现。
- 验证（2026-09-23 更新：绿测已通过，CI/部署未执行）：最终全量 `python -m pytest --tb=short -q` 为 3160 通过 / 169 跳过 / 1 失败（4444.29 秒），不得写全绿；169 跳过为专用 PostgreSQL 条件未配置（并发/恢复/发布数据库等），不计入通过；唯一失败为 `tests/test_legacy_projection_bootstrap.py::test_r2c2_external_writer_lock_is_transient_and_retryable` 的写锁时序敏感超时（当前分支 lock-fixed-1 失败 / lock-fixed-2 通过 15.76 秒；未修改 d36d991 基线 lock-baseline-1 通过 16.30 秒 / lock-baseline-2 失败 22.61 秒；同目录临时还原 HEAD 的 `settlement.py` 通过 16.62 秒并已 `finally` 恢复且字节一致），不改阈值、不修无关写锁模块。第二轮 199 通过 / 2 失败已返修（A-B-A 改走实收字段、双券第二券补实收、非 list 回退漏洞）。定向 `tests/test_receipt_commission_basis.py` + `tests/test_data_settlement_incremental.py` 99 通过（60.54 秒）；导出 API 20 通过 / 103 deselected；管理费 history 浏览器新代码通过、旧代码失败复现后恢复；既有 4 项导出浏览器用例通过；Web build 通过（22.98 秒）；`git diff --check`、治理锁、全局 0 errors / 0 warnings、路由 S4、计划结构一致性通过；索引 33 个已存在断链保持。新增批量查询回归：1001 券触及 SQLite 999 变量上限，旧实现红测 **1 failed / 54 deselected（5.92 秒）**（`logs/dydata97/receipt-batch-red.txt`，`too many SQL variables`）；dsh 已在 `apps/worker/settlement.py::_receipt_amounts_for_details` 按 `RECEIPT_COUPON_BATCH_SIZE = 500` 分批查询并保留整单归属计数，Codex 绿测 `logs/dydata97/receipt-batch-green.txt` 已完成：实收、结算、增量定向 **215 passed（135.06 秒，2026-09-24）**。佣金按实收（0/缺失/损坏/多券/退款/锁账/legacy 月度基数）与管理费导出 `includeHistory` 修复已实现；用户报告的整体导不出尚未复现，已再次请求页面与错误信息，不得声称两类导出全面恢复。2026-09-23 追加只读逐按钮全链路审计并落盘 `logs/dydata97/all-export-audit.md`（25 个按钮；确定缺陷 D1 管理费 includeHistory 已修复、D2 CORS 未 expose_headers、D3 账单异议导出反馈被详情抽屉吞掉、D4 错误行下载无异常处理；高置信候选 C1 同步 revokeObjectURL、C2 100000 行上限 413、C3 榜单导出 300s 超时），不改业务代码、不重跑全量 pytest；逐按钮真实浏览器+真实本地 API 矩阵（V1-V11）与候选定性待 Codex 执行。2026-09-23 追加：`logs/dydata97/all-export-red.txt` 真实本地 FastAPI+SQLite+真实 Vite+真实 Chromium 红测 30 项 = 15 通过 / 15 失败（无 fixture 失败），15 项失败全部为审计已定性的 D2（11：6 文件名 + 5 EMPTY 提示）、D3（2）、D4（2）；**该红测为本地隔离链路，不是生产复现**，生产同源下 D2 不显现。15 项通过对应 14 类按钮成功内容（模板表头、CSV 行数与列表 total 一致、`utf-8-sig` 可解码、门店越权与权限矩阵 403、D1 历史行集一致）已验；15 项失败对应缺陷待修复后验收。已按最小修复改 4 个生产文件（`main.py` CORS `expose_headers` 仅暴露 4 个下载元数据头、`FinanceDisputesPage.tsx` 独立 `exportMessage/exportIsError` 且置于 fixed 抽屉之上、`FinanceImportsPage.tsx` 与 `FinanceImportActionPanel.tsx` 错误行下载 async+busy+try/catch（ImportsPage 成功/失败分别转 `success`/`error`，无 finally）与 try/catch/finally（ActionPanel busy 复位）+失败 `alert`），未改测试断言、C1 共享 revoke 与 C2/C3 均未修改且不在本次部署变更内、未改上限/鉴权/金额；修复后绿测已通过：`tests/test_finance_export_browser.py` 30 项全部通过 119.61s（修复前 15 通过/15 失败 224.21s，逐 14 类实际按钮下载，真实 FastAPI+SQLite+Vite+Chromium，认证 test override，非线上登录验证），相关 API 子集 22 通过/106 deselected（46.91s）、前端契约 39 通过（1.91s）、Web build 23.87s、`git diff --check` 与计划一致性通过；既有 visual_smoke 财务回归34 passed / 228 deselected（197.81秒），CI 与部署尚未执行，不声称通过。主范围 14 类入口映射见测试模块顶部，未扩大到其他 11 个旁路按钮。T1.1 与 Issue 保持进行中 / 已通过绿测、待 CI/部署与用户验收；用户已授权修复后部署，由 Codex 执行验收与部署；无数据库迁移、不自动重算历史锁账，未获业务验收前不关闭。
- 下一步：绿测与 Web build 已完成（`tests/test_finance_export_browser.py` 30 项全部通过、相关回归 22+39 通过、build 23.87s）；既有 visual_smoke 财务回归已通过（`-k "finance or dydata_97"` 34 passed / 228 deselected，197.81 秒，`logs/dydata97/export-existing-browser.txt`）；由 Codex 执行 CI、授权部署与部署后 smoke；按 `logs/dydata97/all-export-audit.md` §6 对未覆盖的 11 个旁路按钮与候选 C1-C3 记录结论或明确未复现（本轮未改 C1/C2/C3，不在部署变更内，不得写成已修）。T1.1 保持进行中，已通过绿测、待 CI/部署与用户验收；不自动重算历史锁账。

# 当前执行：账号激活资料半小时刷新（2026-09-17）

- 用户明确授权跳过 Linear，直接修改。
- 调整门店 POI、账号绑定接口、后台绑定导出的刷新间隔为 30 分钟，并采用半小时任务标识。
- 调度专项 20 项通过；相关回归共 93 项通过（其中 1 项补充本机模块路径后单独复跑通过）。完整回归长时间无输出后停止，尚未提交或部署。
- 证据：[开发日志](../devlog/20260917_activation_half_hour.md)。
...
- 当前 Task：T0.3，进行中（实施、部署与70个账号开户完成，待用户验收）。
- 用户已授权跳过Linear建票，直接修复部署并继续开户。
- 根因：账号权限只读取旧归属表，正式名单仅发布到组织历史表。
- 验证：新增3项用例先失败复现，修复后44项专项通过；完整CI与正式部署35176467356成功，生产70行预览无错误且全部创建成功，下载结果逐项核对一致。证据见[开发日志](../devlog/20260917_account_current_org.md)。

## 本轮账号改造（2026-09-15）

- 已完成计划：[账号组织权限与门店选择](delivery-plans/main-delivery-plan-account-org-scope.md)。
- 追加任务：T0.2，已完成（2026-09-15）；PR #34与部署34937054128成功，新批量账号初始密码123456，组织路径及模板同步修正。记录见[日志](../devlog/20260915_account_default_password.md)。
...
- 当前 Task：T0.1，进行中（已部署、生产恢复验证通过，待用户验收）。
- 实施：独立采样、分级保护、持续恢复、安全检查点让出、非失败计数、后台资源与时效状态。
- 验证：全量3054通过169跳过；真实PostgreSQL发布门禁及17项专项通过；Web和容器构建成功。运行时关键代码哈希一致，服务健康。
- 发布与恢复：1de306c6于2026-09-14 01:01部署，[流水线34768370597](https://github.com/jojiuchen-blip/dy-data/actions/runs/34768370597)成功。08:50核验9月12—13日日批均发布成功，绑定更新至08:42；资源持续normal约7.6小时，残留swap未误停。历史补拉已发布8月18日，正在处理8月17日，尚未全部完成。
- 下一步：用户验收；本次恢复监控已达成目标并暂停，系统既有历史补拉继续。详情见[开发日志](../devlog/20260914_refactor_log_Keith_Chen.md)。
...
- 当前 Task：T0.1，进行中（已部署，待用户验收）；81项相关回归通过，隔离Linux容器启动、三次VNC自动恢复、耗尽后不健康及Chromium会话保持验证通过。主CI3013通过/167跳过，3d5ac35a于9月12日15:53部署，运行时哈希和完整健康探针通过。
- 运行恢复：9月12日13:39绑定导出2956条成功；9月9—11日all日批成功。另发现host swap残留导致资源保护暂停领取新任务，在足够内存余量下回收并重新启用swap，阈值未变。
- 下一步：用户验收；部署后绑定导出2956条成功、失败0，服务健康。DYDATA-90继续承接交换内存保护的恢复条件和整体积压验收。
...
- 当前 Task：T0.1，进行中（已部署，待用户验收）；`34a0f12f` 于2026-09-10 21:32:53部署，流水线34479348382成功，CI 2787 passed / 161 skipped，线上代码与文案核验通过。
- 下一步：用户验收后台“数据同步”的登录失效提示；已有生产登录态保持不变。
...
- 下一步：用户验收；DYDATA-90整体历史积压与72小时观察继续。绑定采集已在用户恢复登录后于9月10日19:58补同步成功：1107条绑定记录，失败0；登录过期误报接口地址缺失的问题已由DYDATA-94修复并部署34a0f12f。下方保留历史记录。
