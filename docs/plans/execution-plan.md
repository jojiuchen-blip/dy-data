...
- T5.7 G3 已完成：0043 固化账单头与订单明细快照，历史缺失值进入异常清单，查询不再回退可变主数据，部署前强制异常归零；迁移隔离专项 3 项、Alembic/部署/前端契约 52 项、Web build、单头与 diff check 均通过，独立复审 Critical/Important 为 0、`Ready: yes`。
- 发布前主线预检：当前分支相对最新 `origin/main` 落后 55、领先 37，预演合并存在 61 个真实冲突及历史迁移内容冲突。G3 冻结后必须从最新主线建立干净集成分支、重建财务迁移链并分块移植；不得在旧分支硬合并后直接发布。
