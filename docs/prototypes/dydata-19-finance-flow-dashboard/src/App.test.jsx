...
describe("门店端财务流程", () => {
  ...
  it("开票后填写发票信息区域显示厂端打款强提醒", () => {
    ...
    expect(screen.getAllByText("开票后必须回系统上传开票信息，否则厂端无法打款")).toHaveLength(2);
...
describe("财务端财务流程", () => {
  ...
  it("财务订单明细将订单券状态及商品SKU分别展示为独立列", () => {
    ...
    for (const header of ["订单", "券", "状态", "商品", "SKU ID"]) {
      expect(screen.getByRole("columnheader", { name: header })).toBeInTheDocument();
    }
  ...
  it("导入记录只保留日志且不再承担业务导入入口", () => {
    ...
    expect(screen.getByText("仅保存导入日志，不保存原始上传文件。", { exact: false })).toBeInTheDocument();
