...
function createHistoricalCycles(now: Date): ClueAllocationCycle[] {
  return Array.from({ length: 8 }, (_, index) => {
    ...
    const cycleMode = index % 3 === 0 ? "trial_rebuild" : "trial";
    const assignedCount = selectedLeadKeys.length - 1;
    const createdAt = shiftedIso(now, -(80 - index * 8));
    return {
      cycle_id: cycleId,
      allocation_cycle_id: cycleId,
      cycle_mode: cycleMode,
      cycle_type: cycleMode,
      execution_mode: "trial",
      cycle_status: "completed",
      status: "completed",
      trigger_type: "manual",
      parent_cycle_id: index % 3 === 0 && index > 0 ? `DEMO-CYCLE-HISTORY-${pad(index, 3)}` : null,
      source_cycle_id: index % 3 === 0 && index > 0 ? `DEMO-CYCLE-HISTORY-${pad(index, 3)}` : null,
      selected_lead_keys: selectedLeadKeys,
      requested_lead_count: selectedLeadKeys.length,
      eligible_lead_count: selectedLeadKeys.length,
      active_lead_count: selectedLeadKeys.length,
      assigned_lead_count: assignedCount,
      headquarters_pool_count: 1,
      skipped_lead_count: 0,
      failed_lead_count: 0,
      planned_impact: { assigned: selectedLeadKeys.length, headquarters: 0, skipped: 0 },
      actual_impact: { assigned: assignedCount, headquarters: 1, skipped: 0 },
...
function createHistoricalAudits(
  ...
  return Array.from({ length: 24 }, (_, index) => ({
    ...
    request_id: `DEMO-REQUEST-${pad(index + 1, 4)}`,
    ...
    if (leadIndex < 360) {
      ...
      if (leadIndex % 8 === 6) {
        ...
        headquartersPool.push({
          ...
          normalized_order_status: outcome.orderStatus,
      ...
      headquartersPool.push({
        ...
        normalized_order_status: "fulfilling",
        raw_order_status: "DEMO_FULFILLING",
        status: leadIndex % 5 === 0 ? "closed" : "active",
        reason: "sale_store_unmapped",
        reason_code: "anchor_store_unmapped",
        reason_label: "锚点门店无法匹配",
