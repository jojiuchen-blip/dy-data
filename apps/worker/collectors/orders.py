    ...
    *,
    ...
    for order in client.iter_orders(window.start, window.end):
        ...
        upsert_raw_order(
            ...
            owner_account_id=text(first(order, "owner_account_id", "sale_info.transfer_uid", "sale_info.account_id")),
            owner_douyin_uid=text(first(order, "owner_douyin_uid", "sale_info.transfer_douyin_uid")),
            owner_account_name=text(
                first(order, "owner_account_name", "sale_info.transfer_nickName", "sale_info.transfer_nickname")
            ),
            sale_role=text(first(order, "sale_role", "sale_info.role")),
            sale_channel=text(first(order, "sale_channel", "sale_info.channel")),
        ...
        for coupon in _coupon_rows(order):
            ...
            upsert_order_coupon(
                ...
                coupon_status=text(first(coupon, "coupon_status", "certificate_status", "status")),
                coupon_updated_at=source_datetime(first(coupon, "coupon_updated_at", "update_time", "updated_at")),
